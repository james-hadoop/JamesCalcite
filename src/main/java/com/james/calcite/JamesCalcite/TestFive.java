...
import com.james.common.aux.DogSqlValidator;
import com.james.common.aux.DogVolcanoPlanner;
import com.james.common.aux.converter.DogFilterConverter;
import com.james.common.aux.converter.DogProjectConverter;
import com.james.common.aux.converter.DogTableScanConverter;
import com.james.common.aux.cost.DogRelMetaDataProvider;
    ...
    public static void main(String[] args) {
        try {
            ...
            final FrameworkConfig config = Frameworks.newConfigBuilder().parserConfig(SqlParser.Config.DEFAULT)
                    .defaultSchema(rootSchema).traitDefs(ConventionTraitDef.INSTANCE, RelDistributionTraitDef.INSTANCE)
                    .build();
            // String sql = "insert into table_result select id+1, name, score from (select
            // a.id + 1 as id, a.name as name, b.score from users a left join score b on
            // a.id = b.id where a.time_d between '2008-09-12' and cast('2015-09-21' as
            // date) + interval '60' second)";

            String sql = "select id, time_d from users where id between id * 5 and id * id";
            // String sql = "select id, time_d from users";
            // String orSql = "select time_d from users where id > 100 or id > 400 and id in
            // (select id from users where id < 500)";
            // String sql = "insert into final_result select id, cast(1 as varchar) as ds,
            // ds as source_ds from my_source";
    ...
    public static RelNode sqlToRelNode(SchemaPlus rootScheme, FrameworkConfig frameworkConfig, String sql) {
        ...
        try {
            ...
            CalciteCatalogReader calciteCatalogReader = new CalciteCatalogReader(
                    ...
                    CalciteSchema.from(frameworkConfig.getDefaultSchema()).path(null), factory,
            ...
            DogSqlValidator validator = new DogSqlValidator(sqlOperatorTable, calciteCatalogReader, factory,
            ...
            final SqlToRelConverter.Config config = SqlToRelConverter.configBuilder()
                    .withConfig(frameworkConfig.getSqlToRelConverterConfig()).withTrimUnusedFields(false)
                    .withConvertTableAccess(false).build();
